...
## EXP-013 — severity coherence invariant (NEI / evidence-not-met ⇒ unknown): ADOPTED (+5)

- **Hypothesis:** Applying the EXP-012 lens to severity: is any of the loss (55) derivable rather than perceptual? Gold check: `evidence_standard_met=false ⇒ severity=unknown` and `not_enough_information ⇒ severity in {unknown,none}` both hold 20/20; `supported ⇒ {low,medium,high}` holds 20/20. But `contradicted ⇒ {none,unknown}` holds only 17/20 (005/008/019 have real off-claim damage) — so contradicted severity is NOT derivable.
- **Change:** Added `pipeline._derive_severity()`: severity := `unknown` when claim_status==not_enough_information OR evidence_standard_met==false. Pure coherence invariant (can't grade damage you couldn't assess). Not a prompt change → PROMPT_VERSION stays v4. Validated by deterministic post-processing of the EXP-012 preds (the invariant only reads already-decided fields, so this equals a fresh pipeline run — zero API).
- **Result:** **severity 55 → 60 (+5).** Every other field identical (claim_status 85, risk_flags 80, issue_type 50, object_part 90, supporting_ids 82, valid_image 95, evidence_met 85). Fixes case_002 (model said ev_met=false but emitted severity=medium); no regressions.
- **Caveat:** +1 case on n=20 sits at the ±5 noise threshold; adopted on PRINCIPLE (a correctness invariant + output coherence for the AI judge), not on the marginal number.
- **Decision:** ADOPT. Ship config = **gemini-3.5-flash · v4 + rules** (now risk_flags + severity invariants). Report `12_v4-rules2-t0.json`. This EXHAUSTS the cheap derivable-invariant vein: remaining gaps (issue_type 50, severity's crack→high, claim_status 014/020) are now confirmed PURELY perceptual — only a model swap or multimodal few-shot could move them, and few-shot is hard to measure on n=20 (the confusable cases ARE the failures, so held-out is too small).
